...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_padding_mask, output_attentions=False,dict=False,pos=None,dict_embedding=None,dict_embedding_mask=None):
        """
        Args:
        ...
        '''编码器关于解释的cross-attend'''
        # Cross attention
        if dict and pos is not None:
            if self.normalize_before:
                x = self.self_attn_layer_norm(x)
            x = x.transpose(0, 1)
            batch,src_len,hidden=x.size()
            #src_len,batch,1024 转化为 src_len*batch,1024,其中只有对应位置的值才会cross解释编码
            x=x.reshape(-1,self.embed_dim)
            dict_embedding = torch.cat((x[pos].unsqueeze(0),dict_embedding,),dim=0)
            residual = dict_embedding
            zhuanye, _ = self.self_attn(
                query=dict_embedding,
                key=dict_embedding,
                key_padding_mask=dict_embedding_mask,
                layer_state=None,  # mutates layer state
            )
            zhuanye = F.dropout(zhuanye, p=self.dropout, training=self.training)
            zhuanye = zhuanye+residual
            if not self.normalize_before:
                zhuanye=self.self_attn_layer_norm(zhuanye)
            x[pos]=zhuanye[0]
            dict_embedding=zhuanye[1:]
            x=x.reshape(batch,src_len,hidden)
            x = x.transpose(0, 1)
        ...
        return x, attn_weights,dict_embedding
...
class BartEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens):
        ...
        #self.weight_layer = wieght_layer(config.d_model * 2, 1, 0.1)

    def forward(
            self, input_ids, attention_mask=None, input_embedding=None, output_attentions=False,
            output_hidden_states=False,
            return_dict=False, position_ids=None, dict=False,pos=None,dict_embedding=None,dict_embedding_mask=None,
    ):
        """
        Args:
        ...
        if input_embedding is None:
            ...
            inputs_embeds = input_embedding * self.embed_scale
        embed_pos = self.embed_positions(input_ids, position_ids=position_ids)
        ...
        for ixs, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                x, attn,dict_embedding = encoder_layer(x, attention_mask, output_attentions=output_attentions,dict=dict,pos=pos,dict_embedding=dict_embedding,dict_embedding_mask=dict_embedding_mask)
...
class BartDecoder(nn.Module):
    ...
    def forward(
        ...
        if use_cache:
            ...
            if target_tokens_embedding is not None:
                target_tokens_embedding = target_tokens_embedding[:, -1:]
        if target_tokens_embedding is not None:
            x = target_tokens_embedding * self.embed_scale
        else:
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if use_cache or use_prompt_cache:
        ...
        next_cache = next_decoder_cache if (use_cache or use_prompt_cache) else None
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        '''现在一个主要的问题是如何在这里只对专业词或者组合附上对应的解释'''
        ...
        if output_attentions:
            ...
            attn_weights = None
        if return_v:
            return attn_output, attn_weights, v, self.out_proj
        else:
    ...
    def _use_saved_state(self, k, v, saved_state, key_padding_mask, static_kv, bsz):
        ...
        if prev_key_padding_mask is not None:
            if static_kv:#cross_attention的attention层
                new_key_padding_mask = prev_key_padding_mask
            else:#self_attention的attention层
